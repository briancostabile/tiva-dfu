...
class DfuFunctionalDescriptor():
    ...
    def __init__(self, data):
        ...
        if (self.bLength == self.LENGTH) and (self.bDescriptorType == self.TYPE):
            ...
            self.wDetachTimeOut, \
            self.wTransferSize, \
            self.bcdDFUVersion = struct.unpack('<BHHH', data[2:])
            self.version = f"{(self.bcdDFUVersion>>8)}.{((self.bcdDFUVersion>>4) & 0x0F)}"
...
class DfuDevice():
    ...
    def send(self, msg):
        ...
        try:
            ret = self.usbDev.ctrl_transfer(msg.bmRequestType,
                                            msg.bmRequest,
                                            wValue=msg.wValue,
                ...
                for intf in config:
                    if intf.bInterfaceClass == 0xFE: # Application Specific
                        if intf.bInterfaceNumber in funcDict.keys():
                            ...
                            dfuDevices.append(DfuDevice(dev, config, intf, name, funcDict[intf.bInterfaceNumber]))
