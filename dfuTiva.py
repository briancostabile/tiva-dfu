...
class DfuResponseTivaQuery:
    ...
    def __init__(self, data):
        if data is None:
            ...
            self.valid = (self.usMarker == self.MARKER) and (
                self.usVersion == self.VERSION
            )
        return
...
class MfgFmt1:
    ...
    def __init__(
        self, productId="None", hwRev=0, serialNum=0, rand0=None, rand1=None, data=None
    ):
        if data is not None:
            self.mfgData = data
            ...
            (
                ...
                self.fmt,
                self.signature,
            ) = struct.unpack("<QQLHHBBBBL", self.mfgData)

            self.valid = (
                (self.signature == self.SIGNATURE)
                and (self.fmt == self.FMT)
                and (self.rsv == 0)
                and (self.bspLen == self.BSP_LEN)
                and (self.usrLen == 0)
            )
        else:
            self.valid = True
            ...
            self.rand0 = (
                rand0
                if rand0 is not None
                else int.from_bytes(os.urandom(8), "little", signed=False)
            )
            self.rand1 = (
                rand1
                if rand1 is not None
                else int.from_bytes(os.urandom(8), "little", signed=False)
            )
            self.productId = int.from_bytes(
                str.encode(productId), "little", signed=False
            )
            self.mfgData = bytearray(
                struct.pack(
                    "<QQLHHBBBBL",
                    self.rand0,
                    self.rand1,
...
class DfuDeviceTiva:
    ...
    def flashBlockErase(self, blkNum):
        ...
        self.dev.tunnelDnloadNoStatus(
            DfuTivaCmdCheck((blkNum * self.cmdBlkMult), self.flashBlockSize)
        )
        status = self.dev.getStatus()
    ...
    def flashErase(self, statusCallback=None):
        ...
        for i in range(self.numFlashBlocks):
            ...
            if statusCallback is not None:
                statusCallback(True, percentComplete)
        if statusCallback is not None:
            statusCallback(True, 100)
        return
    ...
    def flashBlockRead(self, blkNum, statusCallback=None):
        ...
        self.dev.tunnelDnload(
            DfuTivaCmdRead((blkNum * self.cmdBlkMult), self.flashBlockSize)
        )
        ...
        while remain > 0:
            ...
            remain -= readSize
            percentComplete = (
                (self.flashBlockSize - remain) / self.flashBlockSize
            ) * 100
            if statusCallback is not None:
                statusCallback(True, percentComplete)
    ...
    def flashBlockWrite(self, blkNum, blkData, statusCallback=None):
        ...
        if len(blkData) != self.flashBlockSize:
            print(
                "ERROR: blkDataLen:{len(blkData)} != flashBlockSize:{self.flashBlockSize}"
            )
            return
        ...
        if ret is None:
            ...
            if statusCallback is not None:
                statusCallback(False, percentComplete)
        else:
            if statusCallback is not None:
                statusCallback(True, 100)
    ...
    def imageRead(self, statusCallback=None):
        ...
        self.uploadPrefixEnable(False)
        self.dev.tunnelDnload(DfuTivaCmdRead(0, size))
        size += 8
        percentComplete = 0
        while size > 0:
            readSize = self.dfuBlockSize if (size > self.dfuBlockSize) else size
            image += bytearray(self.dev.upload(readSize))
    ...
    def imageVerify(self, statusCallback=None):
        match = True
        size = self.flashTop

        # Remove mfg section from compare image
        cmpImage = self.image[: -MfgFmt1.LEN]
        ...
        while size > 0:
            readSize = self.dfuBlockSize if (size > self.dfuBlockSize) else size
            readBlk = bytearray(self.dev.upload(readSize))

            if firstBlk:
                readBlk = readBlk[8:]
                firstBlk = False
            cmpRemain = len(cmpImage) - offset
            cmpLen = len(readBlk) if (len(readBlk) <= cmpRemain) else cmpRemain
            match = readBlk[:cmpLen] == cmpImage[offset : offset + cmpLen]
            offset += cmpLen

            if not match:
                print(f"Verify Mismatch offset:0x{offset:08X} size:{cmpLen}")

            percentComplete = ((self.flashTop - size) / self.flashTop) * 100
            if statusCallback is not None:
                statusCallback(match, percentComplete)
            size -= readSize

        if statusCallback is not None:
            statusCallback(match, 100)
        return match
    ...
    def imageFlash(self, statusCallback=None):
        for i in range(self.numFlashBlocks):
            ...
            if statusCallback is not None:
                statusCallback(True, ((i / self.numFlashBlocks) * 100))
        if statusCallback is not None:
            statusCallback(True, 100)
        return
...
def main():
    ...
    mfg = tiva.mfgRead()
    print(mfg)
    mfg = MfgFmt1(productId="LPM3", hwRev=0x0304, serialNum=0x0002)
    # print(mfg)
    tiva.mfgWrite(mfg)
    mfg = tiva.mfgRead()
    print(mfg)
